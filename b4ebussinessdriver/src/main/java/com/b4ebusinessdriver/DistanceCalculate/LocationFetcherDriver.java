...
public class LocationFetcherDriver implements ConnectionCallbacks, OnConnectionFailedListener {
    ...
    public void connect() {
        int resp = GooglePlayServicesUtil.isGooglePlayServicesAvailable(this.context);
        if (resp != 0) {
           // Logger.log("Google Play Service Error ", "=" + resp);
    ...
    public void destroy() {
        try {
            ...
            if (this.googleApiClient.isConnected()) {
                ...
                ActivityRecognition.ActivityRecognitionApi.removeActivityUpdates(this.googleApiClient, this.activityIntent);
                this.activityIntent.cancel();
    ...
    protected void createLocationRequest(long interval) {
        ...
        this.locationrequest.setFastestInterval(interval);
    ...
    protected void startLocationUpdates(long interval) {
        ...
        this.locationIntent = PendingIntent.getBroadcast(this.context, LOCATION_PI_ID, new Intent(this.context, LocationReceiverDriver.class), PendingIntent.FLAG_UPDATE_CURRENT);
        LocationServices.FusedLocationApi.requestLocationUpdates(this.googleApiClient, this.locationrequest, this.locationIntent);

        this.activityIntent = PendingIntent.getBroadcast(this.context, LOCATION_PI_ID, new Intent(this.context, ActivityReceiverDriver.class), PendingIntent.FLAG_UPDATE_CURRENT);
        ActivityRecognition.ActivityRecognitionApi.requestActivityUpdates(
                this.googleApiClient, 0,
                this.activityIntent
        );
    }

    public void onConnected(Bundle connectionHint) {
        Logger.log(this.TAG, "onConnected ********************************************************");
        startLocationUpdates(this.requestInterval);
    }

    public void onConnectionSuspended(int i) {
        Logger.log(this.TAG, "onDisconnected ********************************************************");
        destroy();
        connect();
    }

    public void onConnectionFailed(ConnectionResult result) {
        Logger.log(this.TAG, "onConnectionFailed ********************************************************");
        destroy();
        connect();
    }
}
