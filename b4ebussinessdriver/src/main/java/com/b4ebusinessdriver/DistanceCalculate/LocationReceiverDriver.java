...
public class LocationReceiverDriver extends BroadcastReceiver {
    ...
    private static final String RESTART_SERVICE = "com.b4ebusinessdriver.DistanceCalculate.RESTART_SERVICE";
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            ...
            if (!Utils.mockLocationEnabled(location) && location != null) {
                AppPreferences.setCurLat(context, location.getLatitude()+"");
                AppPreferences.setCurLong(context, location.getLongitude()+"");
                ...
                new Thread(new Runnable() {
                    public void run() {
                        ...
                        if (!AppPreferences.getUserId(context2).equalsIgnoreCase("")) {
                            new DriverLocationDispatcher().sendLocationToServer(context2,location,speed_1);
                        }
                        ...
                        in.putExtra(Helper.INTENT_USER_PRE_LAT_LNG, oldlocation);
                        in.putExtra(Helper.INTENT_USER_DISTANCE, Database.getInstance(context2).getTotalDistance());
                        LocalBroadcastManager.getInstance(context2).sendBroadcast(in);
                ...
                if (Prefs.with(context).getInt(Constants.IS_OFFLINE, 0) == 1) {
                    ...
                    Logger.log(this.TAG, "onReceive DriverLocationUpdateService restarted location.getAccuracy()=" + location.getAccuracy());
                    context.stopService(new Intent(context, DriverLocationUpdateService.class));
                    setAlarm(context);
