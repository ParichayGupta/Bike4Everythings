...
import com.b4ebusinessdriver.DistanceCalculate.CurrentPathItem;
import com.b4ebusinessdriver.DistanceCalculate.Log;
...
public class Database {
    ...
    private static final String DATABASE_NAME = "b2bdriver_database";
    ...
    public long insertEmail(String email) {
        ...
        Cursor cursor1 = this.database.query(TABLE_EMAIL_SUGGESTIONS, columns, EMAIL+"=?", new String[]{email}, null, null, null);
        if (cursor1.getCount() > 0) {
            return 1;
        }
        Cursor cursor = this.database.query(TABLE_EMAIL_SUGGESTIONS, columns, null, null, null, null, null);
        int count = cursor.getCount();
        int columnIndex = cursor.getColumnIndex(EMAIL);
        if (count >= 20) {
            String emailToDelete = "";
            cursor.moveToFirst();
            deleteEmail(cursor.getString(columnIndex));
        }
        cursor.close();
        cursor1.close();
        ContentValues contentValues = new ContentValues();
        contentValues.put(EMAIL, email);
        return this.database.insert(TABLE_EMAIL_SUGGESTIONS, null, contentValues);
    }

    public String[] getEmails() {
        Cursor cursor = this.database.query(TABLE_EMAIL_SUGGESTIONS, new String[]{EMAIL}, null, null, null, null, null);
        String result = "";
    ...
    public void deleteEmail(String email) {
        this.database.delete(TABLE_EMAIL_SUGGESTIONS, EMAIL+"=?", new String[]{email});
    }
    ...
    public void updateDriverCurrentLocation(Context context, Location location) {
        ...
        try {
            ...
            Logger.log("insert successful", "= rowId =" + this.database.insert(TABLE_DRIVER_CURRENT_LOCATION, null, contentValues));
        } catch (Exception e) {
            try {
                e.printStackTrace();
                Logger.log("e", "=" + e);
